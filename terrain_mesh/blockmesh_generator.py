...
class BlockMeshGenerator:
    ...
    def generate_blockMeshDict(self, config: MeshConfig, input_vtk_file: str = "terrain_structured.vtk", output_dict_file: str = "system/blockMeshDict", inlet_face_file: str ="0/include/inletFaceInfo.txt"):
        ...
        if config.is_using_legacy_z_config():
            # Legacy approach
    ...
    def _blockMeshDictCreatorWithZGrading(self,
        ...
        """
        class MockConfig:
            def __init__(self, first_cell_height, z_grading, total_z_cells):
                self.first_cell_height = first_cell_height
                self.z_grading = z_grading
                self.total_z_cells = total_z_cells
            
            def is_using_legacy_z_config(self):
                return False

        mock_config = MockConfig(first_cell_height, z_grading, total_z_cells)
        ...
        z_grading_spec = self._calculate_z_grading_spec(
            domain_height, first_cell_height, z_grading, total_z_cells
        )
        ...
        try:
            ...
            if first_cell_height:
                print(f"  First cell height: {first_cell_height}m")
    ...
    def _blockMeshDictCreator(self,
        ...
        """
        class MockConfig:
            def __init__(self, num_cells_z, expansion_ratio_z):
                self.num_cells_z = num_cells_z
                self.expansion_ratio_z = expansion_ratio_z
            
            def is_using_legacy_z_config(self):
                return True

        mock_config = MockConfig(num_cells_z, expansion_ratio_R)
    ...
    def save_inlet_face_info(self, block_positions, boundary_patches, points, output_file, 
        ...
        with open(output_file, 'w') as f:
            ...
            if mesh_config.is_using_legacy_z_config():
                f.write("mesh_type=legacy\n")
                f.write(f"num_cells_z={mesh_config.num_cells_z}\n")
                f.write(f"expansion_ratio_z={mesh_config.expansion_ratio_z}\n")
            else:
                f.write("mesh_type=graded\n")
                f.write(f"total_z_cells={mesh_config.total_z_cells}\n")
                if mesh_config.first_cell_height is not None:
                    f.write(f"first_cell_height={mesh_config.first_cell_height}\n")
                
                # Write z_grading as parseable format
                f.write("z_grading=")
                grading_str = ";".join([f"{spec[0]},{spec[1]},{spec[2]}" for spec in mesh_config.z_grading])
        ...
        print(f"Included mesh parameters: {'legacy' if mesh_config.is_using_legacy_z_config() else 'graded'}")
        return inlet_faces
    ...
    def _calculate_z_grading_spec(self, domain_height: float, first_cell_height: float, 
                            z_grading: List[Tuple[float, float, float]], total_z_cells: int) -> str:
        """
        Calculate the z-direction grading specification string for OpenFOAM blockMeshDict.
        
        Args:
            domain_height: Total domain height
            first_cell_height: Height of first cell (optional)
        ...
        if first_cell_height is None:
            # No first cell, use standard multiGrading
            if len(z_grading) == 1:
            ...
            # With first cell - need to adjust grading for remaining domain
            first_cell_frac = first_cell_height / domain_height
            remaining_height_frac = 1.0 - first_cell_frac
            
            # Calculate cells for first layer (always 1 cell)
            ...
            remaining_cell_frac = remaining_cells / total_z_cells
            
            # Build multiGrading specification
            grading_parts = []
            
            # First layer (1 cell, no expansion)
            grading_parts.append(f"({first_cell_frac} {first_layer_cell_frac} 1)")
            
            # Remaining layers with scaled parameters
            for length_frac, cell_frac, expansion_ratio in z_grading:
                scaled_length_frac = length_frac * remaining_height_frac
                scaled_cell_frac = cell_frac * remaining_cell_frac
                grading_parts.append(f"({scaled_length_frac} {scaled_cell_frac} {expansion_ratio})")
