...
def create_blockMesh_spacing(n_points: int, grading_spec: List[Tuple[float, float, float]]) -> np.ndarray:
    """
    ...
    for length_frac, cell_count, expansion_ratio in zip(length_fractions, actual_cells, expansion_ratios):
        if cell_count == 0:
            continue
...
def calculate_z_coordinates(domain_height: float, ground_height: float, 
                           first_cell_height: float, z_grading: List[Tuple[float, float, float]], 
                           total_z_cells: int) -> np.ndarray:
    """
    Calculate z-coordinates accounting for first cell height if specified.
    Matches the logic in blockmesh_generator.py
    """
    ...
    # With first cell height
    first_cell_frac = first_cell_height / domain_height
    remaining_height_frac = 1.0 - first_cell_frac
    
    first_layer_cell_frac = 1.0 / total_z_cells
    remaining_cells = total_z_cells - 1
    
    if remaining_cells == 0:
        return np.array([ground_height, ground_height + first_cell_height])
    
    remaining_cell_frac = (total_z_cells - 1) / total_z_cells
    
    # Build adjusted grading for remaining cells
    adjusted_grading = []
    for length_frac, cell_frac, expansion_ratio in z_grading:
        scaled_length_frac = length_frac * remaining_height_frac
        scaled_cell_frac = cell_frac * remaining_cell_frac
        adjusted_grading.append((scaled_length_frac, scaled_cell_frac, expansion_ratio))
    
    # Create normalized coordinates for remaining domain
    remaining_norm = create_blockMesh_spacing(remaining_cells + 1, adjusted_grading)
    
    # Combine first cell and remaining cells
    z_coords = np.zeros(total_z_cells + 1)
    z_coords[0] = ground_height
    z_coords[1] = ground_height + first_cell_height
    z_coords[2:] = ground_height + first_cell_height + remaining_norm[1:] * (domain_height - first_cell_height)
...
def load_config(config_path: str):
    ...
    if 'x_grading' in config['grid']:
        config['grid']['x_grading'] = [tuple(spec) for spec in config['grid']['x_grading']]
    if 'y_grading' in config['grid']:
        config['grid']['y_grading'] = [tuple(spec) for spec in config['grid']['y_grading']]
    if 'z_grading' in config['mesh']:
        config['mesh']['z_grading'] = [tuple(spec) for spec in config['mesh']['z_grading']]
...
def visualize_mesh(config_path: str, ground_height: float, output_dir: str = None):
    ...
    if 'total_z_cells' in mesh:
        ...
        first_cell_height = mesh.get('first_cell_height', None)
        z_grading = mesh['z_grading']
        legacy_mode = False
    else:
        total_z_cells = mesh['num_cells_z']
        first_cell_height = None
        z_grading = [(1.0, 1.0, mesh['expansion_ratio_z'])]  # Convert to grading format
        legacy_mode = True
    ...
    # Z direction
    z_coords = calculate_z_coordinates(domain_height, ground_height, 
                                       first_cell_height, z_grading, total_z_cells)
    ...
    # Print statistics
    print(f"\nMesh Statistics:")
    print(f"  Domain: {domain_size/1000:.1f} x {domain_size/1000:.1f} x {domain_height:.0f} m")
    print(f"  Ground height: {ground_height:.0f} m")
    print(f"  Grid: {nx} x {ny} x {total_z_cells}")
    print(f"  X cells: min={x_cells.min():.2f}m, max={x_cells.max():.2f}m, ratio={x_cells.max()/x_cells.min():.2f}")
    print(f"  Y cells: min={y_cells.min():.2f}m, max={y_cells.max():.2f}m, ratio={y_cells.max()/y_cells.min():.2f}")
    print(f"  Z cells: min={z_cells.min():.2f}m, max={z_cells.max():.2f}m, ratio={z_cells.max()/z_cells.min():.2f}")
    print(f"  Total cell count = {nx*ny*total_z_cells/1000000:.1f}M cells")
    if first_cell_height:
        print(f"  First cell height: {first_cell_height:.2f}m")
    ...
    for j in range(nx):
        ax2.plot([x_coords[j]] * (total_z_cells + 1), z_coords, 'b-', linewidth=0.5, alpha=0.6)
    ax2.axhline(y=ground_height, color='brown', linestyle='--', linewidth=1.5, label='Ground', alpha=0.7)
    ax2.set_xlabel('X (m)', fontsize=11)
    ax2.set_ylabel('Z (m)', fontsize=11)
    ax2.set_title(f'XZ Slice (Elevation View) - Y={y_coords[center_y_idx]:.0f}m', 
    ...
    ax5.axhline(y=ground_height, color='brown', linestyle='--', linewidth=1.5, alpha=0.7)
    if first_cell_height:
        ax5.axhline(y=ground_height + first_cell_height, color='orange', 
                   linestyle='--', linewidth=1, alpha=0.7, label='First cell')
...
def main():
    ...
    parser.add_argument('--ground-height', type=float, required=True,
